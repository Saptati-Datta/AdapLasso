...
```
In order to find the optimal $\gamma$ value, we can perform another cross-validation using the ```cv.gamma``` function. We find the pair of  ($\lambda,\gamma$) corresponding to which the entry in the ```cvm``` matrix is minimum.
```{r}
